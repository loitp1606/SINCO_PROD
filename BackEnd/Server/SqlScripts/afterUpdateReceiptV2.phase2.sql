ALTER PROCEDURE [dbo].[afterUpdateReceiptV2]
  @idGui NVARCHAR(50),
  @vcNum NVARCHAR(50),
  @VCDate NVARCHAR(50)
AS
BEGIN
    ...
    FROM receiptV2$000000
    WHERE idgui = @idGui;

    SELECT TOP 0 a.*, @sync AS sync
    INTO #detail
    FROM receiptdetailV2$000000 a
    WHERE idGui = @idGui;

    SELECT TOP 0 idGui, voucherDate, CAST('' AS VARCHAR(6)) AS p
    INTO #partitionPoin
    FROM receiptV2$000000;
    ...
    UPDATE #detail
    SET sync = CONVERT(VARCHAR(6), a.voucherDate, 112)
    FROM deliveryNote$000000 a
    JOIN #detail b ON a.idGui = b.idGuiDN;

    INSERT INTO #partitionPoin
    ...
    SELECT @partition = MIN(p) FROM #partitionPoin;
    WHILE @partition IS NOT NULL
    BEGIN
        SELECT @q = '
            update deliveryNoteDetail$' + @partition + '
                set sl_receipt = isnull(sl_receipt, 0) + isnull(b.amount, 0) from deliveryNoteDetail$' + @partition + ' a join #detail b on a.idGui = b.idGuiDN and a.line_nbr = b.lnDN
        ';
        PRINT @q;
        EXEC (@q);
        SELECT @partition = MIN(p) FROM #partitionPoin WHERE p > @partition;
    END;
    ...
    BEGIN
        ...
        if exists (select 1 from deliveryNoteDetail$' + @sync + ' where sl_receipt <> 0 and idGui = ''' + @idSL + ''')
            update deliveryNote$' + @sync + N' set status = ''1'', status_name = N''Đã giao hàng'', deliveryStatus = ''PARTIAL'', trang_thai_duyet2 = ''1'', ten_trang_thai2 = N''Đang chuyển phiếu thu'' where idGui = ''' + @idSL + '''
        ';
        SELECT @q = @q + N'
        if not exists (select 1 from deliveryNoteDetail$' + @sync + ' where amount > sl_receipt and idGui = ''' + @idSL + ''')
            update deliveryNote$' + @sync + N' set status = ''1'', status_name = N''Đã giao hàng'', deliveryStatus = ''DELIVERED'', trang_thai_duyet2 = ''2'', ten_trang_thai2 = N''Hoàn thành chuyển phiếu thu''  where idGui = ''' + @idSL + '''
        ';
        EXEC (@q);

        EXEC UpdateDeliveyNoteFromReceipt @idSL, 'UP';
        ...
        BEGIN
            ...
            BEGIN
                ...
                EXEC sp_executesql @sqlPhase2, N'@p_idSL varchar(50)', @p_idSL = @idSL;
            END;
        END;
