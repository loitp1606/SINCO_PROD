CREATE OR ALTER PROCEDURE [dbo].[oot$exportVatInvoice]
...
BEGIN
    ...
    (
        SELECT
            r.*,
            MaHDValue = 'HD' + RTRIM(r.stt),
            TienHangValue = SUM(ISNULL(r.amount, 0)) OVER (PARTITION BY r.idGui),
            TienThueValue = SUM(ROUND(ISNULL(r.amount, 0) * ISNULL(r.tax_rate, 0) / 100, 0)) OVER (PARTITION BY r.idGui),
            ThueSuatValue = MAX(ISNULL(r.tax_rate, 0)) OVER (PARTITION BY r.idGui)
        FROM #Raw r
    )
    SELECT
        ...
        MaHD = r.MaHDValue,
        SoSeriKyHieuHD = COALESCE(NULLIF(r.invoiceVAT, N''), N'C23TND'),
        SoPhieuHoaDon = r.MaHDValue,
        ...
        DienGiai = N'Bán hàng',
        MaDonVi = COALESCE(NULLIF(@unit, N''), N'CONGTY'),
        TienHang = r.TienHangValue,
        TienThue = r.TienThueValue,
        TongTien = r.TienHangValue + r.TienThueValue,
        LoaiHD = 1,
        HTTT = N'TM/CK',
        ...
        HinhThucThanhToan = N'TM/CK',
        ThueSuat = r.ThueSuatValue,
