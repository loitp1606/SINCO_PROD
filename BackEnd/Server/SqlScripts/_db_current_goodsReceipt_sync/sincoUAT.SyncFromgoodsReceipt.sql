...
CREATE   PROCEDURE [dbo].[SyncFromgoodsReceipt]
    @Ids NVARCHAR(MAX),
    @Unit NVARCHAR(50),
    @UserId NVARCHAR(50),
...
BEGIN
    ...
    BEGIN
        SELECT 0 AS type, N'Không có phi?u nh?p d? k? th?a.' AS message;
        RETURN;
    END;
...
';
    ...
    BEGIN
        SELECT 0 AS type, N'Không tìm th?y d? li?u phi?u nh?p.' AS message;
        RETURN;
    END;
    ...
    BEGIN
        SELECT 0 AS type,
               CASE WHEN @Language = 'V'
                    THEN N'Có mã nhà cung c?p khác nhau gi?a các phi?u nh?p, vui lòng xem l?i!!!'
    ...
    BEGIN
        ...
        SET @q = N'
            ...
            SELECT
                @p_newid,
                ROW_NUMBER() OVER(ORDER BY m.idGui),
                MAX(m.voucherNumber) AS invoiceNumber,
                MAX(m.voucherDate) AS invoiceDate,
                SUM(totalAmount) AS invoiceAmount,
                SUM(paidAmount) AS paidAmount,
                SUM(debtAmount) AS debtAmount,
                '''' AS accountPaymentCode,
                SUM(debtAmount) AS amountCur,
                SUM(debtAmount) AS amount,
                MAX(m.note) AS note,
                MAX(m.idGui) AS idGuiPN,
                1 AS lnPN
            FROM #master m
            GROUP BY m.idGui;
        ';
        ...
        BEGIN
            SELECT 0 AS type, N'Phi?u nh?p dã k? th?a h?t sang phi?u chi!!' AS message;
            RETURN;
        END;
    ...
    BEGIN
        ...
        BEGIN
            SELECT 0 AS type, N'Chua có b?ng orderReturn/orderReturnDetail.' AS message;
            RETURN;
        END;
        ...
        UPDATE #orderReturnMt SET customerGroupCode = b.industry_group_id, phoneCustomer = b.phone_number from #orderReturnMt a join supplier b on a.customerCode = b.supplier_id
        ...
        DECLARE @exprQty NVARCHAR(200) = CASE
            WHEN COL_LENGTH('tempdb..#detail', 'quantity') IS NOT NULL THEN N'ISNULL(TRY_CONVERT(decimal(24,4), d.quantity), 0)-ISNULL(TRY_CONVERT(decimal(24,4), d.sl_tl), 0)'
            ELSE N'0'
        END;
        ...
        ';
        EXEC sp_executesql @q, N'@p_newid varchar(64)', @p_newid = @newid;
		delete #orderReturnDt where quantity <=0

        UPDATE #orderReturnDt set quantity = round(quantity,2), price = round(price,2), amount = round (amount,0)
        , discount = round (discount,0), tax = round (tax,0), payment = round (payment,0)

        IF NOT EXISTS (SELECT 1 FROM #orderReturnDt)
        BEGIN
            SELECT 0 AS type, N'Phi?u nh?p dã k? th?a h?t sang phi?u xu?t tr?.' AS message;
            RETURN;
        END;
        ...
        (
            SELECT
                idGui,
                SUM(ISNULL(round(quantity,2), 0)) AS totalQuantity,
                SUM(ISNULL(round(amount,0), 0)) AS totalAmount,
                SUM(ISNULL(round(tax,0), 0)) AS totalTax,
                SUM(ISNULL(round(discount,0), 0)) AS totalDiscount,
                SUM(ISNULL(round(payment,0), 0)) AS totalPayment
            FROM #orderReturnDt
            GROUP BY idGui
    ...
    SELECT 0 AS type,
           CASE WHEN @Language = 'V'
                THEN N'Chua h? tr? c?u hình k? th?a này.'
